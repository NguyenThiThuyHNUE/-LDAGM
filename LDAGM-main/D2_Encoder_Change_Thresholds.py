...
if __name__ == "__main__":
    ...
    # Tìm ngưỡng tốt nhất theo F1-score
    best_f1 = 0
    best_threshold = 0.5
    for t in thresholds:
        temp_preds = np.array([1 if p > t else 0 for p in pre_target])
        f1 = f1_score(test_target, temp_preds)
        if f1 > best_f1:
            best_f1 = f1
            best_threshold = t

    # In ra ngưỡng tốt nhất
    print(f"✅ Ngưỡng tối ưu theo F1-score: {best_threshold:.4f}")

    # Áp dụng ngưỡng tối ưu để phân loại
    preds = np.array([1 if p > best_threshold else 0 for p in pre_target])
